...
def examine_text_content(tmp_text, idx, flags, pro_yes=False):
    ...
    pro_pattern = re.compile(
        r"^(.*)\b(remove|without|disable|block).+(ad|ads|advertisement|advertisements|advertising|adverts)\b"
    )
    mo1 = pro_pattern.search(tmp_text)

    pro_pattern2 = re.compile(
        r"^(.*)\b(no ad|no ads|no advertisement|no advertisements|no advertising|noadverts|ad free|no-ad)\b"
    )
    ...
    watch_pattern = re.compile(
        r"^(.*)\b(watch).*\b(ad|ads|advertisement|advertisements|advertising|adverts)\b"
    )
    ...
    google_pattern = re.compile(
        r"^(.*)(get|download|find|app).*(on)(.+)(app store|google play| )(.*)"
    )
    ...
    rate_pattern = re.compile(
        r"^(.*)(do|if).*(you).*(like|enjoy|love)(.+)(rate|star|google play|play store|app store|app)(.*)"
    )
    ...
    free_pattern = re.compile(
        r"^(.*)(try|get|start|enjoy).*(month|week|day)(.+)(free)(.*)"
    )
    upgrade_pattern = re.compile(
        r"(.*)(upgrade|download the prenium|free trial).*(premium|)"
    )
    mo1 = free_pattern.search(tmp_text)
    mo2 = upgrade_pattern.search(tmp_text)
    if (
        not (mo1 is None)
        or "premium" in tmp_text
        or "upgrade" in tmp_text.strip()
        or (not (mo2 is None))
    ):
    ...
    # multiple currencies
    if ("coin" in tmp_text.split() or "coins" in tmp_text.split()) and len(
        tmp_text.split()
    ) < 5:
    ...
    sub_pattern = re.compile(
        r"^.+(free|free trial).+(then|continue).+($| )\d+.*(year|month|week|forenight).*"
    )
    ...
    privacy_pattern = re.compile(
        r"^(.*)(you|i|).*(consent|agree|give consent|accept)(.+)(terms|terms of use|privacy policy|policies|terms and conditions|terms & conditions|terms of service|license agreement)(.*)"
    )
    ...
    notification_pattern = re.compile(
        r"^(.*)(push|enable|links in|daily|allow|turn on|receive|get).*(notification|newsletter|news|update|message|email).*"
    )
    mo1 = notification_pattern.search(tmp_text)

    notification_pattern2 = re.compile(
        r"^(.*)(notify|notification|newsletter|news).*"
    )  # update|message|email|ringtone|sound|vibration|vibrate
    ...
    ## usage data
    usage_data_pattern = re.compile(
        r"^(.*)(usage tracking|analytics|usage data|usage statistics|usage report|collect information|analytics purpose|anonymous stats|usage info).*"
    )
    ...
    ## no checkbox
    privacy_pattern = re.compile(
        r"^(.*)(by| )(.*)(continuing|tapping|using|creating|signing|clicking|pressing|logging|installing|logging|taping).*(allow|agree|consent|accept|confirm)(.*)"
    )  # (terms of service|terms of use|privacy policies|privacy policy|terms|terms and conditions|user agreement|terms of user agreement|cookie policy|use policy|terms & conditions|privacy statement|license agreement|rule| )(.*)
    mo1 = privacy_pattern.search(tmp_text)
    ...
    daily_pattern = re.compile(
        r"^(.*)(daily|weekly|nightly).*(reward|bonus|dozon)(.*)(.*)"
    )
...
def examine_compound(item, idx, flags):
    ...
    if item_category in ["ImageView", "ImageButton"] and item_text.strip() in [
        "ad",
        "ads",
    ]:
...
def get_info(item_list, item_ids, subType, flag):
    ...
    for item_id in item_ids:
        ...
        tmp_info = {
            "type": category2eleType[item_category],
            "bbox": item_bbox,
            "subType": subType,
            "flag": flag,
            "text": item.get("text", ""),
    ...
    for tmp_flag in ["flag_rate"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[NG] A rating page pops up unexpectedly, interrupt user tasks and nags users.",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_premium"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[NG] An ungrade page unexpectedly pops up and nags users to upgrade to premium version",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_coin", "flag_real_money"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[OB-IntCur] Intermediate Currency",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_sub"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[SN-FC] Forced Continuity - The app provides a free trial, but it forces users to consent to auto-subscribe after the end of the free trial. ",
                tmp_flag,
            )
...
def check_interface_inference(flags, item_list, output_results):
    ...
    for tmp_flag in ["flag_pre_privacy_by"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[PRE] Users are forced to consent to privacy policies/terms of use as there is no checkbox",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_pre_privacy"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[PRE] The privacy policy/terms of use is consent by default",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_pre_notification"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[PRE] Enable notification by default",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_pre_usage_data"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[PRE] The app automatically sends users' usage data for analysis by defauly",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_pre_follow"]:
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[PRE] Follow some accounts by default",
                tmp_flag,
            )
    ...
    for tmp_flag in ["flag_offer"]:  # "flag_emoji",
        if tmp_flag in flags:
            ...
            tmp_re = get_info(
                item_list,
                flags[tmp_flag],
                "[II-AM-TWE] Countdown Offer/Rewards to make users nervous",
                tmp_flag,
            )
    ...
    for tmp_flag in [
        ...
        if tmp_flag in flags:
            if "AD" not in output_results:
                output_results["AD"] = []
            tmp_re = get_info(
                item_list, flags[tmp_flag], "[AD] Ad Icons/Texts", tmp_flag
            )
    ...
    if "flag_notClearAd" in flags:
        ...
        tmp_re = get_info(
            item_list,
            flags[tmp_flag],
            "[DA] Icon/Button is ad, but it is not clear",
            "flag_notClearAd",
        )
    ...
    if "flag_smallClose_icon" in flags:
        ...
        tmp_re = get_info(
            item_list,
            flags[tmp_flag],
            "[AM] The close icon in ad is so small that users may mistakenly open the ad.",
            tmp_flag,
        )
...
def check_forced_action(flags, item_list, output_results):
    ...
    if "flag_friend" in flags:
        ...
        tmp_re = get_info(
            item_list,
            flags[tmp_flag],
            "[FA] Users can obtain some rewards by invitating their friends, which puts social pressure to their friends",
            "flag_friend",
        )
    ...
    if "flag_watch" in flags:
        ...
        tmp_re = get_info(
            item_list,
            flags[tmp_flag],
            "[FA] Users are forced to watch Ads to unlock some features",
            "flag_watch",
        )
    ...
    if "flag_pro" in flags:
        ...
        tmp_re = get_info(
            item_list,
            flags[tmp_flag],
            "[FA-G-PRO] Users have to pay to avoid ads",
            "flag_pro",
        )
    ...
    if "flag_daily" in flags:
        ...
        tmp_re = get_info(
            item_list, flags[tmp_flag], "[FA-GAMIFICATION] daily awards", "flag_daily"
        )
...
def final_check(output_results, img_h, img_w, all_items):
    ...
    if typ in output_results:
        ...
        if len(typ_items) > 0:
            ...
            for itt in typ_items:
                children.append(
                    {"label": typ, "bbox": itt["bbox"], "flag": itt["flag"]}
                )
            ...
            y_center = (y1 + y2) // 2
            if (
                img_w // 3 <= x_center <= 2 * img_w // 3
                and img_h // 3 <= y_center <= 2 * img_h // 3
            ):
                # if True:
                data = {
                    "label": typ,
    ...
    for item in item_sorted:
        ...
        if item_cate != "check_group":
            if "notification" in item_text or "usage" in item_text:
                ...
                if flag_notification_block and (
                    not is_horizontal_overlap(noti_block_bbox, item["bbox"])
                ):
                    flag_notification_block = False
        else:
    ...
    if "AD" in output_results:
        ...
        AD_INFO_ITEMS = [
            item
            for item in typ_items
            if item["flag"] in ["flag_adInfo_icon", "flag_info_icon"]
        ]
        AD_Play_ITEMS = [
            item for item in typ_items if item["flag"] == "flag_adPlay_icon"
        ]
        CLOSE_ITEMS = [
            item
            for item in typ_items
            if item["flag"] in ["flag_close_icon", "flag_ad_continue"]
        ]
        COUNTDOWN_ITEMS = [
            item
            for item in typ_items
            if item["flag"] in ["flag_countdown", "flag_countdown_ad"]
        ]
        ...
        for item in [*AD_TEXT_ITEMS, *AD_INFO_ITEMS, *AD_Play_ITEMS]:
            ...
            if flag_top_right or flag_bottom_left:
                if (
                    not ("FA-G-PRO" in final_results and item["flag"] == "flag_ad_text")
            ...
            # print("==", item)
            if (
                item["flag"] not in ["flag_adPlay_icon", "flag_info_icon"]
                and not flag_ng_ad
                and len(output_results.get("II-AM-G-SMALL", [])) == 0
            ):
        ...
        if len(potential_ng_ads) > 0:
            ...
            for idx in range(len(potential_ng_ads[1:])):
                if is_horizontal_overlap(
                    potential_ng_ads[idx - 1]["bbox"], potential_ng_ads[idx]["bbox"]
                ):
...
def draw_results(img_path, output_results, item_list, vis_save_root):
    ...
    for DP_type, instances in output_results.items():
        ...
        for idx, item in enumerate(instances):
            ...
            combined_img = cv2.rectangle(
                combined_img, (x1, y1), (x2, y2), color=curr_color, thickness=5
            )
            count += 1
            ...
            circle = cv2.resize(
                circle, (circle_d, circle_d), interpolation=cv2.INTER_AREA
            )
            combined_img[
                max(0, y1 - circle_d) : max(0, y1 - circle_d) + circle_d,
                max(0, x1 - circle_d)
                + x_offset : max(0, x1 - circle_d)
                + circle_d
                + x_offset,
            ] = circle
            x_offset = 0
            ...
            combined_img[
                count * line_gap - diff : count * line_gap + (circle_d - diff),
                w + 3 : w + 3 + circle_d,
            ] = circle
            ...
            for text_line_id in range(text_lines):
                curr_text = subtype[
                    text_line_id
                    * num_text_per_line : (text_line_id + 1)
                    * num_text_per_line
                ]
                # print(curr_text[-1], text_line_id, subtype[(text_line_id+1)*num_text_per_line])
                if (
                    curr_text[-1] != " "
                    and text_line_id != text_lines - 1
                    and subtype[(text_line_id + 1) * num_text_per_line] != " "
                ):
                    curr_text += "-"
                combined_img = cv2.putText(
                    combined_img,
                    curr_text,
                    (w + 30 + 40, 30 + count * line_gap),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    fontScale=TextScale,
                    color=curr_color,
                    thickness=thickness,
    ...
    vis_path = os.path.join(
        vis_save_root, os.path.basename(img_path).split(".")[0] + "-dp.jpg"
    )
    # print(vis_path)
